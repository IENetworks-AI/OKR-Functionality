                              ...
                              return (
                                <div key={taskKey} className="p-2 border rounded flex justify-between items-center">
                                  <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">Priority: {task.priority} | Weight: {task.weight}% | Target: {task.target}%</p>
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <Input
                                      type="number"
                                      value={task.achieved}
                                      onChange={(e) => handleUpdateTask(plan.id, task.id, 'achieved', parseFloat(e.target.value) || 0)}
                                      className="w-20"
                                    />
                                    <Button
                                      size="sm"
                                      onClick={() => handleSaveTask(plan.id, task)}
                                    >
                                      Save
                                    </Button>
                                    <Button
