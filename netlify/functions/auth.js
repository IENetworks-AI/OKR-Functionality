exports.handler = async (event) => {
  ...
  try {
    ...
    if (!email || !password || !apiKey) {
      console.error('Missing required env or payload:', { email, password, apiKey });
      return {
        statusCode: 400,
    ...
    try {
      ...
      console.error('Failed to parse auth response as JSON:', err);
      return {
        statusCode: 500,
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ idToken: data.idToken, expiresIn: data.expiresIn, raw: data }),
    };
  } catch (error) {
